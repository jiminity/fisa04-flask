...
from flask import Blueprint, url_for, render_template, flash, request, session, g #g: global의 앞글자, 전역적으로 틀라이언트와 서버를 드러나르는 역할
...
import functools # 우리가 만든 함수를 @어노테이션을 통해서 먼저 실행하도록 함
...
# 요청에 대해 응답 전에 flask가 자체적으로 무조건 검증하고 가는 로직을 위한 어노테이션
...
# 로그인을 통한 인가가 필요할때 @login_required를 먼저 실행
